# Name: 05_downloadData_GSE49604-GPL10558.R
...
oExp =  getGEO(filename = 'dataExternal/gse49604/GSE49604-GPL10558_series_matrix.txt.gz')
...
f = grepl('OA', dfSamples$characteristics_ch1.3)
group1[f] = 'OA'
group2 = rep('Untreated', times=nrow(dfSamples))
f = grepl('IL1B', dfSamples$characteristics_ch1.1)
group2[f] = 'IL1B'
...
oExp$fTreatment = dfSamples$group2
...
getwd()
n = make.names(paste('GSE49604-GPL10558 fibroblast-like synoviocytes treated and untreated with IL1B from RA and OA.rds'))
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='GSE49604-GPL10558 fibroblast-like synoviocytes, treated and untreated with IL1B from RA and OA. This study has another data set check for data id 22.')
